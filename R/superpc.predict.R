superpc.predict <- function(object, data, newdata, threshold, n.components = 3, prediction.type = c("continuous", "discrete", "nonzero"), n.class = 2) {
    ...
    if (prediction.type == "continuous") {
        ...
        # v.pred.1df <- cut(v.pred.1df, breaks = br, labels = FALSE) v.pred.1df[is.na(v.pred.1df)] <- 1
        v.pred.1df = ifelse(v.pred.1df <= br[2], 1, 2)  ## yp
    }
    if (is.matrix(out)) {
        dimnames(out) = list(NULL, rep(prediction.type, ncol(out)))
    }
    junk <- list(v.pred = out, u = x.sml.svd$u, d = x.sml.svd$d, which.features = which.features, v.pred.1df = v.pred.1df, n.components = n.pc, coef = result$coef, call = this.call, prediction.type = prediction.type)
    return(junk)
}
