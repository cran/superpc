superpc.rainbowplot = function(data, pred, sample.labels, competing.predictors, call.win.metafile = FALSE) {
    ...
    layout(matrix(1:npanels, ncomp.predictors + 4, 2, byrow = TRUE), widths = c(0.8, 0.2), heights = c(0.2, rep(0.1, (ncomp.predictors + 2)), 0.05))
    # layout.show(14)
    par(mar = c(2, 0, 1, 0))
    par(cex = 0.8)
    o = order(pred)
    ...
    for (i in 1:n) {
        text(i/n, 0.5, labels = sample.labels[i], srt = 90, cex = 0.5)
    }
...
my.barplot = function(x, label, type = c("continuous", "discrete"), col = c("red", "green", "blue", "gray")) {
    ...
    if (type == "continuous") {
        if (col == "gray") {
            palette(gray(seq(0, 0.9, length = n)))
            cols = rank(x)
        }
        ...
        nc = 4
        temp = quantile(x, c(0.25, 0.5, 0.75, 1))
        values.legend = paste("<", round(temp, 2), sep = "")
        cols.legend = sort(cols[trunc(n * c(0.25, 0.5, 0.75, 1))])
    ...
    text(0.5, 0.7, label = label, cex = 0.8)
    xval = c(0, 1/nc, 1/nc, 0)
    yval = c(0, 0, h, h) + h
    for (i in 1:nc) {
        polygon(xval, yval, col = cols.legend[i])
        text((xval[1] + xval[2])/2, yval[1] - 0.2, labels = values.legend[i], cex = 0.5)
        xval = xval + 1/nc
    }
...
surv.to.class2 <- function(y, icens, cutoffs = NULL, n.class = NULL, class.names = NULL, newy = y, newic = icens) {
    # this is the function 'pamr.surv.to.class2' from the pamr libarary the auxiliary functions below are also from pamr
    # Splits patients into classes based on their survival times The user can either specify the number of classes or the survival time cutoffs.  y - vector of survival times icens - censoring
    # indicator cutoffs - survival time cutoffs n.class - number of classes to create class.names - optional vector of names for each class
...
gen.y.mat2 <- function(surv.data, cutoffs, class.names = NULL, newdata = surv.data) {
    # Calculates the probability that a given patient belongs to a given class.  Returns a matrix where entry (i,j) is the probability that patient i belongs to class j.  The function calculates
    # the probability that a given patient dies between two given cutoffs, and uses this information to calculate the probability that a patient with a censored survival time died in a given
    # interval.
...
get.surv.q <- function(surv.obj, quantile) {
    ...
    if (ndx == 0) 
        return(0) else return(surv.obj$time[ndx])
}
