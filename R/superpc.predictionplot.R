superpc.predictionplot <- function(train.obj, data, data.test, threshold, n.components = 3, n.class = 2, shrinkage = NULL, call.win.metafile = FALSE) {
    ...
    if (is.null(shrinkage)) {
        fit.cts = superpc.predict(train.obj, data, data.test, threshold = threshold, n.components = n.components, prediction.type = "continuous")
    ...
    if (train.obj$type == "survival") {
        ...
        if (n.components == 1) {
            layout(matrix(c(1, 2), 1, 2, byrow = TRUE), widths = c(0.8, 0.2), heights = c(1, 1))
        }
        
        if (n.components == 2) {
            layout(matrix(c(1, 4, 2, 5, 3, 6), 3, 2, byrow = TRUE), widths = c(0.8, 0.2), heights = c(0.34, 0.33, 0.33))
        }
        
        if (n.components == 3) {
            layout(matrix(c(1, 5, 2, 6, 3, 7, 4, 8), 4, 2, byrow = TRUE), widths = c(0.8, 0.2), heights = rep(0.25, 4))
        }
        ...
        for (i in 1:n.components) {
            ...
            if (i == 1 & n.class == 2) {
                legend(1.05 * max(data.test$y), 0.8, lty = c(1, 1), col = 2:lastc, c("low score", "high score"), cex = 0.8)
            }
            
            if (i == 1 & n.class == 3) {
                legend(1.05 * max(data.test$y), 0.8, lty = c(1, 1, 1), col = 2:lastc, c("low score", "med score", "high score"), cex = 0.8)
            }
        ...
        if (n.components > 1) {
            # write out combined 1df predictor results for(i in 1:(n.components-1)){ plot(0,0,type='n',axes=F,xlab='',ylab='') }
            plot(0, 0, xlim = c(0, 1), ylim = c(0, 1), type = "n", axes = F, xlab = "", ylab = "")
    ...
    if (train.obj$type == "regression") {
        layout(matrix(c(1, 2), 1, 2, byrow = TRUE), widths = c(0.6, 0.4), heights = c(1, 1))
