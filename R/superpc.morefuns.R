...
coxfunc <- function(x, y, censoring.status, s0.perc) {
...
coxscor <- function(x, y, ic, offset = rep(0, length(y))) {
    # computes cox scor function for rows of nx by n matrix x first put everything in time order
    n <- length(y)
    nx <- nrow(x)
    yy <- y + (ic == 0) * (1e-05)
    otag <- order(yy)
    ...
    ic <- ic[otag]
    x <- x[, otag, drop = F]
    # compute unique failure times, d=# of deaths at each failure time, dd= expanded version of d to length n, s=sum of covariates at each failure time, nn=#obs in each risk set,
...
coxvar <- function(x, y, ic, offset = rep(0, length(y)), coxstuff.obj = NULL) {
    # computes information elements (var) for cox x is nx by n matrix of expression values
    nx <- nrow(x)
    n <- length(y)
    yy <- y + (ic == 0) * (1e-06)
    otag <- order(yy)
...
ocoxvar <- function(x, y, ic, offset = rep(0, length(y)), coxstuff.obj = NULL) {
    # computes information elements (var) for cox x is nx by n matrix of expression values
    nx <- nrow(x)
    n <- length(y)
    yy <- y + (ic == 0) * (1e-06)
