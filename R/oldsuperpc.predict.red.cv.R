    ...
    for (i in 1:num.reduced.models) {
        if (type == "survival") {
            ...
            if (!is.null(junk$fstat)) {
                lrtest.reduced[i] <- junk$fstat[1]
            }
        }
        
    }
    
    return(list(shrinkages = shrinkages, lrtest.reduced = lrtest.reduced, n.components = n.components, num.features = fitred$num.features, v.preval.red = cur.vall, sign.wt = sign.wt, type = type, 
        call = this.call))
}
