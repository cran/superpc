...
superpc.xl.listgenes.compute <- function(data, train.obj, fit.red, fitred.cv = NULL, num.genes = NULL, component.number = 1) {
    ...
    if (is.null(fitred.cv)) {
        foo = list(gene.ids = res[, 1], gene.names = res[, 2], gene.scores = res[, 3:4], gene.headings = c("ID", "Name", " Importance score", "Raw score"))
    }
    if (!is.null(fitred.cv)) {
        foo = list(gene.ids = res[, 1], gene.names = res[, 2], gene.scores = res[, 3:6], gene.headings = c("ID", "Name", " Importance score", "Raw score", "av-rank-in-CV", "prop-selected-in-CV"))
    }
...
superpc.xl.fit.to.clin <- function(fit, data.test, score, pamr.xl.test.sample.labels, pamr.xl.clindata, pamr.xl.clinsamplabels, pamr.xl.clinprednames, pamr.xl.clinpredtype) {
    ...
    # strip off extra first element, row, col that rServer seems to include put everything in same order as test expression data
...
superpc.xl.decorrelate <- function(data, pamr.xl.train.sample.labels, pamr.xl.clindata, pamr.xl.clinsamplabels, pamr.xl.clinprednames, pamr.xl.clinpredtype) {
    ...
    # strip off extra first element, row, col that rServer seems to include put everything in same order as training expression data
    errorflag = FALSE
...
superpc.xl.rainbowplot = function(data, pred, pamr.xl.test.sample.labels, pamr.xl.clindata, pamr.xl.clinsamplabels, pamr.xl.clinprednames, pamr.xl.clinpredtype, call.win.metafile = FALSE) {
